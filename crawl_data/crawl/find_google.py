...
from crawl_data.crawl.wiki_content import crawl_content_wiki
            ...
            async with session.get(url, timeout=30) as response:
                if response.status == 200:
                    ...
                    try:
                        ...
                        with open("crawl_data/crawl/log/log2.txt", "a", encoding="utf-8") as f:
                            f.write(original_name + "\n")
                        return False, None
                elif response.status == 429:
                    with open("crawl_data/crawl/log/log429.txt", "a", encoding="utf-8") as f:
                        f.write(original_name + "\n")
                    return False, None
                else:
...
async def find_and_crawl():
    ...
    with open("crawl_data/crawl/log/log_name_en.txt", "r", encoding="utf-8") as f:
        en = [line.strip() for line in f.readlines()]
    
    with open("crawl_data/crawl/log/log_name_vn.txt", "r", encoding="utf-8") as f:
        vn = [line.strip() for line in f.readlines()]

    with open("crawl_data/crawl/log/log429", "r", encoding="utf-8") as f:
    ...
    total_list = []
    for ani in animalds:
        with open(f"crawl_data/crawl/list_name/{ani}.txt", "r", encoding="utf-8") as f:
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        with open("crawl_data/crawl/log/log429.txt", "w", encoding="utf-8") as f:
        #xoá len(log429) dòng đâu tiên trong log429.txt
            for i in range(len(log429)):
                f.write("")
        with open("crawl_data/crawl/log/log_name_en.txt", "w", encoding="utf-8") as f:
            for name in log429:
                f.write(name + "\n")
        with open("crawl_data/crawl/log/log_name_vn.txt", "a", encoding="utf-8") as f:
            f.write("")
        print("Done")
